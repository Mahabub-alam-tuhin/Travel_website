        ...
        <div class="card-datatable table-responsive">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                ...
                <table class="datatables-users table border-top dataTable no-footer dtr-column" id="DataTables_Table_0"
                    ...
                    <tbody>
                        @php $i=1 @endphp
                        @foreach ($saveresorts as $resort)
                        {{-- @dd($divisions) --}}
                            <tr>
                                <td>{{ $i++ }}</td>
                                <td>{{ $resort->divisions->name ??''}}</td>
                                <td>{{ $resort->district->name ?? '' }}</td>
                                <td>{{ $resort->upazila->name ?? ''}}</td>
                                <td>{{ $resort->union->name ?? '' }}</td>
                                <td>{{ $resort->guid_id ?? '' }}</td>
                                <td>{{ $resort->name }}</td>
                                <td>{{ $resort->day }}</td>
                                <td>{{ $resort->person }}</td>
                                <td>{{ $resort->phone }}</td>
                                <td>{{ $resort->age }}</td>
                                <td>
                                {{ \Illuminate\Support\Str::limit($resort->description, 50, $end='...') }}
                                </td>
                                <td>{{ $resort->entry_fee }}</td>
                                <td>{{ $resort->image }}</td>
                         

                                <td>
                                    <a href="{{route('admin.resort_Management.edit',$resort->id)}}" height="275px" ; width="183px" ;  class="btn btn-primary">Edit</a>
                                    <a href="{{route('admin.resort_Management.delete',$resort->id)}}"  class="btn btn-danger">Delete</a>
                                </td>
                            </tr>
                        @endforeach
                        </thead>
